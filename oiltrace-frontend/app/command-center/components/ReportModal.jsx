      ...
      <div className="bg-white w-full max-w-4xl max-h-[92vh] rounded-2xl overflow-y-auto border border-slate-300 shadow-2xl flex flex-col text-slate-800 font-sans">
        ...
        <div id="printable-report" className="p-8 space-y-6 text-xs text-slate-700 font-sans">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono">
              <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                ...
                <div className="text-sm font-bold text-sky-700">{slickData.areaKm2} kmÂ²</div>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                <div className="text-slate-500 text-[10px]">Est. Release Volume</div>
                <div className="text-sm font-bold text-amber-700">{slickData.volumeM3} mÂ³</div>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
          ...
          <div className="space-y-3">
            ...
            <p className="leading-relaxed text-slate-700">
              Applying a 2D Lagrangian particle drift physics model forced by surface ocean currents ({hindcastData.driftVector.currentSpeedKnots} kts @ {hindcastData.driftVector.currentDirDeg}Â°) and 3.5% wind drift ({hindcastData.driftVector.windSpeedKnots} kts @ {hindcastData.driftVector.windDirDeg}Â°), the slick release origin point was traced backward <span className="text-rose-700 font-bold">{slickData.estimatedAgeHours} hours</span> to coordinates <span className="text-slate-900 font-mono font-semibold">[{hindcastData.originCentroid[0].toFixed(4)}Â°N, {hindcastData.originCentroid[1].toFixed(4)}Â°E]</span> with estimated discharge timestamp <span className="text-slate-900 font-mono">{new Date(slickData.estimatedReleaseTime).toUTCString()}</span>.
            </p>
          </div>
                ...
                <tbody className="divide-y divide-slate-200 bg-white">
                  {rankedVessels.map((v, i) => (
                    <tr key={v.vesselId} className={i === 0 ? 'bg-rose-50 text-rose-900' : 'text-slate-700'}>
                      ...
                      <td className="p-2.5">{v.darkShipGapFound ? `âš ï¸ Yes (${v.gapDurationMinutes}m)` : 'No'}</td>
          ...
          {topSuspect && (
            <div className="bg-rose-50 p-5 rounded-2xl border-2 border-rose-300 space-y-2">
              ...
              <div className="text-sm font-bold text-slate-900 font-mono">
                {topSuspect.vesselName} (IMO {topSuspect.imo} â€¢ MMSI {topSuspect.mmsi})
              </div>
          ...
          <div className="border-t border-slate-200 pt-4 flex justify-between items-center text-[10px] font-mono text-slate-500">
            <div>CONFIDENTIAL LEGAL EVIDENCE DOSSIER â€¢ GENERATED BY AQUASENTINEL AI ENGINE</div>
