        ...
        if (attributionInfo.darkShipGapFound && attributionInfo.gapNearOrigin) {
          ...
          for (let i = 0; i < sorted.length - 1; i++) {
            if (sorted[i + 1].timestamp - sorted[i].timestamp > 30 * 60 * 1000) {
              ...
              ).addTo(layersGroup).bindTooltip(`âš ï¸ AIS Transponder Blackout (${attributionInfo.gapDurationMinutes} mins)`, { permanent: false });
            }
          }
        }
          ...
          const vesselIcon = L.divIcon({
            ...
            html: `
              <div class="relative flex items-center justify-center cursor-pointer transform hover:scale-125 transition-transform">
                ...
                ${
                  isTopSuspect
                    ? `<span class="absolute -top-2 -right-2 px-1 py-0.5 bg-rose-600 text-[9px] font-bold text-white rounded-full shadow">ðŸš¨ ${attributionInfo.masterScore}%</span>`
                    : ''
                }
              </div>
            `,
